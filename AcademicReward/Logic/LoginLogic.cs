...
public class LoginLogic : ILogic {
    ...
    public string GenerateStoredPassword(string password) {
        ...
        RNGCryptoServiceProvider saltStorage = new();
        byte[] salt = new byte[SaltByteSize];
        saltStorage.GetBytes(salt);
        //Creating the hash
        Rfc2898DeriveBytes hasher = new(password, salt, HashIterationCount);
    ...
    public bool IsValidLogin(string storedSalt, string storedPassword, string testPassword) {
        ...
        Rfc2898DeriveBytes hashTool = new(testPassword, storedSaltBytes, HashIterationCount);
